...
- [x] **(Tier 3)** No loyalty/points, no customer tags (VIP, bridal, allergy notes as structured data rather than a free-text `notes` field), no birthday/anniversary reminders — all common salon retention levers. — done 2026-07-25, in the stated sub-order:
  - **Tags**: `customers.tags` (JSON, validated against a fixed vocabulary in `Customer::TAGS` — VIP/Bridal/Regular/Allergy Sensitive) rather than a free-text field or a separate tags table; the vocabulary is small and salon-fixed, so a table with admin-managed CRUD would be more machinery than the problem needs. Editable via checkboxes on the admin Customers add/edit form; shown as chips on each row.
  - **Points**: new `customer_points` ledger table, same shape as `stock_movements` — signed `points` delta + running `balance_after` cached on `customers.points_balance`. Earned automatically off the amount paid on a job (`JobController::addPayment`, `Customer::earnPoints()`, rate set by `config('loyalty.currency_per_point')`, tip excluded); admin can also record a manual `redeemed` or `adjustment` entry via `POST /admin/customers/{customer}/points` (`CustomerPointController`). Admin UI: `CustomerPointsPanel.tsx`, an expandable panel under each customer (same pattern as `StockLedgerPanel.tsx`). Removing a job payment doesn't claw back the points it earned — documented simplification, see the comment in `JobController::addPayment`.
  - **Birthday/anniversary reminders**: `customers.date_of_birth`/`anniversary_date` (nullable dates, editable on the same form) plus `last_birthday_reminder_sent_at`/`last_anniversary_reminder_sent_at` to keep the send idempotent per calendar year. New `customers:send-occasion-reminders` Artisan command (`Console\Commands\SendCustomerOccasionReminders`), scheduled daily at 08:00 in `routes/console.php`, emails a `CustomerOccasionReminder` mailable to any customer with an email address whose birthday/anniversary is today and hasn't had this year's reminder sent yet. **Note:** like the rest of the app's scheduling, this needs `php artisan schedule:run` wired into a cron entry or supervisor job on the production host/container before it actually fires — nothing currently calls it (see `docker-entrypoint.sh`); tracked alongside the "queue the email sending" gap below.
...
3. [ ] **Customers / Jobs / Inventory / Reporting gaps** — re-sorted 2026-07-24 by actual implementation effort (checked against the current schema/models/controllers), not by category. Work top-down within each tier; flip boxes in the sections above as items land.
   ...
   - **Tier 3 — moderate (new table, but isolated):**
     ...
     - [x] Customers: tags → points → birthday/anniversary reminders (in that sub-order) — 2026-07-25
