...
- [x] Build `Api/Admin/*` controllers mirroring the 17 existing `Admin/*` Blade controllers 1:1, but returning JSON instead of views. **17 of 17 done** — all Blade admin controllers now have a JSON counterpart under `Api/Admin/*` with routes wired in `routes/api.php`. See `ADMIN-MIGRATION-TASKS.md` for the one deliberate deviation (a new `GET /users/unlinked-staff` endpoint, since that data was baked into Blade's create/edit view instead of being its own route).
...
- [~] Rebuild each admin screen as a React page/module. **Done: Login, Dashboard, Services, Products, Gallery, Appointments, Wedding Albums, Jobs.** Remaining 8 (Staff, Customers, Orders, Testimonials, Contact Messages, Reports, Activity Log, Users) are `ComingSoon` placeholders in the sidebar. Old Blade views still running in parallel as designed.
- [x] `barryvdh/laravel-dompdf` usage (invoices/receipts) — kept server-side as decided. Jobs receipt (preview/download) works through the new API now, streamed to the React app as a blob since the endpoint needs the Sanctum Bearer token (can't be a plain `<a href>`). Orders invoice still needs the same treatment once that module is ported.
