...
API + scaffold the React admin") — **this phase is done, see below.** Update
this file in the same commit as any module you finish — check the box, add
the date/commit note. This is the single source of truth for Phase 2's
"what's done vs what's left"; SAAS-ROADMAP.md only tracks phase-level
progress, not per-module state.

For the salon-operations feature/enhancement backlog we're working through
next (before Phase 3), see `SALON-OPS-ENHANCEMENTS.md` instead — that's its
own tracker now, not folded into this one.
...
**Next: the salon-operations feature/enhancement backlog in `SALON-OPS-ENHANCEMENTS.md`**, worked through before Phase 3 (multi-tenancy).
